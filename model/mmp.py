...
class MultiModesPreferenceEstimation(object):
    ...
    def get_graph(self):
        ...
        with tf.variable_scope('keys'):
            ...
            user_keys = tf.reshape(tf.matmul(user_embeddings, user_key_weights), [-1, self.key_dim, self.mode_dim])
        ...
        with tf.variable_scope('encoding'):
            ...
            user_latent = tf.tensordot(tf.transpose(attention, perm=[0, 2, 1]), item_embeddings, axes=[[2],[0]])

        with tf.variable_scope('decoding'):
            prediction = tf.tensordot(user_latent, tf.transpose(item_embeddings), axes=[[2], [0]])
            self.prediction = tf.reduce_max(tf.transpose(prediction, perm=[0, 2, 1]), axis=2)

            print self.prediction.shape

        with tf.variable_scope('loss'):
            l2_loss = tf.nn.l2_loss(item_key_weights) + tf.nn.l2_loss(user_key_weights)
    ...
    def get_batches(self, rating_matrix, batch_size):
        ...
        print type(rating_matrix), index.reshape(remaining_size, 1).shape
...
def mmp(matrix_train, embeded_matrix=np.empty((0)),
    ...
    model = MultiModesPreferenceEstimation(matrix_train.shape[1], rank, 5, 10, 100,
